<template>
  <div class="hello">
        ...
        <el-container class="mainViewShell">
            <el-aside :class="isCollapse? 'mainViewShell-aside mainViewShell-aside-close' : 'mainViewShell-aside'">
              ...
              <div class="asideNavMenuView">
                ...
                text-color="#fff">
                  <el-menu-item index="0" @click="identNavigate('Home')">
                    <!-- <template slot="title"> -->
                  ...
                  <el-menu-item index="1" @click="identNavigate('CollectionIndex')">
                    <!-- <template slot="title"> -->
                      ...
                      <span slot="title">数字基坑</span>
                    <!-- </template> -->
                  </el-menu-item>

                  <!-- <el-menu-item index="2" @click="identNavigate('ProjectImg')">
                    <template slot="title">
                      <i class="el-icon-menu"></i>
                  ...
                  <!-- <el-menu-item index="5" @click="identNavigate('ChartsDetailPageForSensor')">
                    <template slot="title">
                  ...
                  <!-- <el-menu-item index="9" @click="identNavigate('StationFrequency')">
                    <template slot="title">
                      <i class="el-icon-news"></i>
                      <span slot="title">采集设置</span>
                    </template>
                  </el-menu-item> -->

                  <!-- <el-submenu v-if="userInfo.power == '管理员'" index="5">
                    <template slot="title">
                      <i class="el-icon-coordinate"></i>
                      <span slot="title">读数仪信息</span>
                    </template>
                    <el-menu-item-group>
                      <el-menu-item v-for="(item,index) in readMeteDataList" 
                        :key="index" index="'5-' + index" 
                        @click="identNavigateSim('ChartsDetailPageForSensor',item.sim,item.id)">
                        {{item.sim}}
                      </el-menu-item>
                  ...
                  <el-menu-item v-if="userInfo.power == '管理员'" index="7" @click="identNavigate('UpLoad')">
                    <template slot="title">
                  ...
                  <el-menu-item v-if="userInfo.power == '管理员'" index="8" @click="identNavigate('FactoryList')">
                    <template slot="title">
                      <i class="el-icon-tickets"></i>
                      <span slot="title">厂家列表</span>
                    </template>
                  </el-menu-item> -->
                    ...
                    <div class="headerStyle-div-flex">
                      <div class="headerStyle-div-flex-leftview">
                        ...
                        <div class="leftview-cItem">
                          <!-- <span v-if="isNotIndex" style="display:inline-block;margin-right:20px">当前项目: {{currentProject}}</span> -->
                          <span style="display:inline-block;margin-right:20px">{{curretnTime}}</span>
                          <span>{{todayWeather.text_day}}&nbsp;&nbsp;{{todayWeather.low}}{{todayWeather.low ? '~' : ''}}{{todayWeather.high}}{{todayWeather.low ? '℃' : ''}}&nbsp;&nbsp;{{todayWeather.wd_day}}</span>
                      ...
                      <div class="headerStyle-div-flex-rightview">
                        ...
                        <!-- 去掉用户名上的点击事件 -->
                        <!-- <div class="leftview-cItem" @click="changeIsShowUserSetting"> -->
                        <div class="leftview-cItem">
    ...
    <el-dialog title="添加读数仪" :visible.sync="dialogFormVisible" center width="400px">
      <el-form>
        <!-- <el-form-item label="sensorCode" :label-width="formLabelWidth"> -->
          <el-input v-model="addSensorForm.sensorCode" autocomplete="off" style="margin-bottom:20px" placeholder="请输入sensorCode"></el-input>
        <!-- </el-form-item> -->
        <!-- <el-form-item label="密码" :label-width="formLabelWidth"> -->
          <el-input v-model="addSensorForm.password" autocomplete="off" placeholder="请输入密码"></el-input>
        <!-- </el-form-item> -->
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogFormVisible = false">取 消</el-button>
        <el-button type="primary" @click="submitAddSensor()">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
import {timeapple,Json2Params,getLocationInfo,responseBack} from '../publicJs/utils'
...
export default {
  ...
  data () {
    return {
      ...
      // mainBackImage: require('@/assets/frontImage.jpg'), 
      mainBackImage: '', // get main back image
      alarmTypeList: [
        {
          name: '正常',
          actived: true,
      ...
      dialogFormVisible:false, //添加读数仪弹框的显示与隐藏
      addSensorForm:{
        sensorCode:'',
        password:''
      },
      curretnTime:'',  //页面上显示的时间
      todayWeather:{}, //今日天气
      isNotIndex:false,  //当前页面是否是首页
    }
  },
  computed: {
    userInfo() {
  ...
  methods: {
    ...
    getCurrentTime(){
      ...
      this.curretnTime = time1
    },
    // 获取天气
    getWeather(){
      this.$axios.get('/found/user/getWeather').then(res => {
        if(res.data.status == 2000){
          this.todayWeather = res.data.body.result.forecasts[0]
        }
      })
      // let param = {
      //     method:'GET',
      //     url:'/weather/v1/',
      //     params:{
      //       district_id:'330108', //滨江区的行政区划编码
      //       data_type:'all',
      //       ak:'iQIb9m7xE2z6H5vVO74rVlFFukaUwU6A'
      //     }
      // } 
      // request(param).then(res => {
      //     if(res.data.message == 'success'){
      //       this.todayWeather = res.data.result.forecasts[0]
      //     }
     ...
     */
    submitAddSensor(){
      if(this.addSensorForm.sensorCode == '' || this.addSensorForm.password == ''){
        this.$message.warning('请输入读数仪的编号和密码')
        return
      }
      ...
      this.$axios.post('/found/factory/createReadMeter',{
          sim: this.addSensorForm.sensorCode,
          password: this.addSensorForm.password,
          userId: userInfoForLocal.id
      }).then(res => {
          // showmessage(this,'创建成功')
          this.$message.warning('创建成功')
          this.dialogFormVisible = false
          this.addSensorForm.sensorCode == ''
          this.addSensorForm.password == ''
          this.listReadMeteList();
      })
    },
    /**
    ...
    listReadMeteList() {
      ...
      this.$axios.get('/found/factory/selectReadMeterByUserId?userId='+userInfoForLocal.id).then(res => {
        if(res.data.status == 2000) {
          this.readMeteDataList = res.data.body
        }
      })
      // let groupListResult = await requestBase.baseApi('/found/factory/selectReadMeterByUserId','GET',{
      //     userId: userInfoForLocal.id
      // })
      // let groupListResult = [{

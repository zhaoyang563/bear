    ...
    <div class="collectionIndex" @click.stop="hideSelectToast">
            ...
            <div class="mainView-topView-flex">
                <div class="mainView-topView-flex-left">
                    <my-select 
                        ...
                        @dataChange="listGroupListBySubProjectId"></my-select>
                    <my-select 
        ...
        <div class="collectionIndexView">
            <div class="alarmSetView">
                <div class="alarmSetViewLeft" v-if="false">
                ...
                <div class="alarmSetViewRight">
                    <div class="maindataview">
                        ...
                        <div class="areachoice">
                              <el-table
                                :data="reportList"
                                :header-cell-style="{background:'rgba(24, 44, 74, 0.5)',color:'#fff'}"
                                style="width: 100%">
                                <el-table-column
                                label="采集日期"
                                width="180">
                                ...
                                width="180">
                                <template slot-scope="scope">
                                    <a class="clear-line" href="" @click.prevent="getReportDetail(scope,'nav')">{{ scope.row.type }}</a>
                                </template>
                                </el-table-column>
                                <el-table-column label="报表操作">
                                <template slot-scope="scope">
                                    <el-button
                                    size="mini"
                                    @click="getReportDetail(scope,'download')">下载</el-button>
                                    <!-- <el-button
                                    size="mini"
                                    type="danger"
...
export default {
    ...
    mounted () {
        this.initTimeValues();
        this.listSubProjectsByMainProject();
        // this.initProject();
        // this.initShowTime();
    ...
    methods: {
         ...
         * 获取主项目下的所有子项目
         */
        listSubProjectsByMainProject() {
            let mainProject = localStorage.getItem('projectInfo')
            if(mainProject == null || mainProject == '') {
                showmessage(this,'主项目没有选择');
                return;
            }
            mainProject = JSON.parse(mainProject);
            this.$axios.get('found/subproject/selectAllSubproject?projectId=' + mainProject.projectId).then(res => {
                console.log('获取子项目')
                console.log(res)
                if(res.data.status == 2000) {
                    // 1.0 获取成功返回的子项目
                    let subProjectList = res.data.body;
                    let subProjectListCal = []
                    subProjectList.forEach(subProjectItem => {
                        if(subProjectItem.status != 0) {
                            let obk = {
                                id: '',
                                name: '',
                                isChecked: false
                            }
                            obk.id = subProjectItem.id;
                            obk.name = subProjectItem.type;
                            subProjectListCal.push(obk)
                        }
                    })
                    console.log('计算后的子项目')
                    console.log(subProjectListCal)
                    this.illusoryDeviceList.deviceList.list = subProjectListCal
                    this.illusoryDeviceList.deviceList.current = subProjectListCal[0]
                    // 根据当前选中的项目 来获取分组的列表
                    this.listGroupListBySubProjectId();
                }
            })
        },
        /**
         * 根据子项目获取分组信息
         */
        listGroupListBySubProjectId() {
            if(!this.illusoryDeviceList.deviceList.current) {
                return
            }
            this.$axios.get('found/group/selectGroupBySubprojectId?subprojectId=' + this.illusoryDeviceList.deviceList.current.id).then(res => {
                console.log('获取子项目下的所有分组信息')
                console.log(res)
                if(res.data.status == 2000) {
                    // 1.0 获取成功返回的子项目
                    let subProjectList = res.data.body;
                    ...
                    subProjectList.forEach(subProjectItem => {
                        ...
                        obk.id = subProjectItem.id;
                        obk.name = subProjectItem.groupName;
                        subProjectListCal.push(obk)
                    })
                    console.log('计算后的子项目')
                    console.log(subProjectListCal)
                    this.illusoryDeviceList.stationList.list = subProjectListCal
                    this.illusoryDeviceList.stationList.current = subProjectListCal[0]
                    // 根据当前选中的项目 来获取分组的列表
                    this.listReportListByGroupId();
                }
            })
        },
        /**
         * 获取报表列表 
         */
        listReportListByGroupId() {
             this.$axios.get('found/report/selectReportList?groupId=' + this.illusoryDeviceList.stationList.current.id
             + '&beginTime=' + new Date(this.value1[0]).getTime() + '&endTime='+ new Date(this.value1[1]).getTime()).then(res => {
                console.log('获取分组下的所有报表')
                console.log(res)
                if(res.data.status == 2000) {
                    // 获取到了报表信息
                    let reportList = res.data.body;
                    reportList.forEach(reportItem => {
                        reportItem.createTime = timeapple(new Date(reportItem.planTime),'all')
                    })
         ...
         * 获取报表的详情
         */
        getReportDetail(currentReportId,type) {
            console.log(currentReportId)
            let userInfo = localStorage.getItem('logininfo')
            userInfo = JSON.parse(userInfo)
            this.$axios.get('found/report/buildReport?planId=' + currentReportId.row.id
             + '&userId=' + userInfo.id + '&flag=1').then(res => {
                console.log('获取报表详情')
                console.log(res)
                if(res.data.status == 2000) {
                   if(type == 'download') {
                       ...
                       this.$router.push({
                           name:'ReportDetail',
                           query: {
                               context: res.data.body.content,
                               type: currentReportId.row.type,
                           }
                       })
                   }
                }
...
.el-table{
    ...
    .el-table /deep/ td, .el-table /deep/ th.is-leaf{

...
export default {
  ...
  data () {
    return {
      ...
      isChangeFrontView: true, // is show front view
  ...
  methods: {
    // 通过点击地图的定位传过来的项目ID获取图片
    getProjectImg(){
      let projectId = this.$route.query.projectId
      // let href = window.location.href
      // let query = href.substring(href.indexOf('?') + 1);
      // console.log(query) 
      // let projectId = query.match(/=(\S*)/)[1];
      console.log(projectId) 
      if(projectId != '' || projectId != null){
        this.$axios.get('found/photo/selectMainMap?projectId='+projectId).then(res => {
          console.log(res)
          if(res.data.status == 2000){
            this.mainBackImage = res.data.body[0].url
          }
        })
      }
    },
    ...
    setMainBackImage(image) {
      ...
      return () => {
        ...
        console.log(this.mainBackImage)
      }
    },
    /**
     * 点击切换设备
    ...
    listReadMeteList() {
      ...
      //     userId: userInfoForLocal.id
      // })
      // console.log('get read meter list')
